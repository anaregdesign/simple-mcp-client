...
export function createAzureSessionHandlers(
  ...
  return {
    async handleAzureLogin(): Promise<void> {
      ...
      try {
        await runAzureLoginFlow(deps, catalogHandlers);
      } catch (loginError) {
    ...
    async handleAzureTenantChange(nextTenantIdRaw: string): Promise<void> {
      const currentState = deps.readState();
      if (
    ...
    async handleAzureLogout(): Promise<void> {
      const currentState = deps.readState();
      if (
        currentState.isStartingAzureLogout ||
        currentState.isSwitchingAzureTenant
    ...
    async handleReloadAzureCatalog(): Promise<void> {
      const currentState = deps.readState();
      if (
        currentState.isAzureAuthRequired ||
