...
export function buildDatabaseDebugLatestThreadToolDescription(): string {
  const lines = [
    ...
    "- `snapshot.thread`: Thread core metadata. Includes parsed `chatAzureConfig`, `threadEnvironment`, and `instructionContextToggles` alongside raw JSON columns and `agentConversationId`.",
...
export async function readDatabaseDebugLatestThreadSnapshot(
  ...
  const threadRecord = {
    ...
    chatAzureConfigJson: thread.chatAzureConfigJson,
    chatAzureConfig: normalizeUnknownForJson(
      readJsonValue(thread.chatAzureConfigJson, null),
    ),
